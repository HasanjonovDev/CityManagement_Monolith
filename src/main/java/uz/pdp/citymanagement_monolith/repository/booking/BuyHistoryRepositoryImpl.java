...
public class BuyHistoryRepositoryImpl extends SimpleJpaRepository<BuyHistoryEntity, UUID> implements BuyHistoryRepository {
    ...
    @Override
    public Optional<BuyHistoryEntity> findBySeller(UserEntity seller) {
        try {
            String findBySeller = "select f from buy_history f where f.seller.id = :sellerId";
            TypedQuery<BuyHistoryEntity> query = entityManager.createQuery(findBySeller, BuyHistoryEntity.class);
            query.setParameter("sellerId",seller.getId());
            return Optional.of(query.getSingleResult());
        } catch (Exception e) {
            log.warn("Error at BuyHistoryRepositoryImpl findBySeller -> {}",e.getMessage());
